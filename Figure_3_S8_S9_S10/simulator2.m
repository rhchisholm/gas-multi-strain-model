function [SSPrev,AgentsInfectedByKStrains, ...
    Observed_AC_time, Observed_ImS_time,...
    AgentCharacteristics, ImmuneStatus,params] = ...
    simulator2(AgentCharacteristics, ImmuneStatus, params, burnin, Consultations, time_obs)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Store population at observation times %%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                 
Observed_AC_time = zeros(Nagents , Nstrains, length(time_obs));
Observed_ImS_time = Observed_AC_time;

Observed_AC_time(:,:,1) = AgentCharacteristics(:,1:end-1);
Observed_ImS_time(:,:,1) = ImmuneStatus;
...
for i = 1 : Ntimesteps - 1  
        ...
        if StrengthCrossImmunity > 0
         ...
         for j = 1 : length(RowIndexInfected)
            ...
            if X > 0
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % % SLOW WAY: Sample X agents with replacement (using 
                % % pregenerated random numbers) from IAgentsAll, which is 
                % % the row indices of all other agents:
                % IAgentsAll = [1:RowIndexInfected(j)-1,RowIndexInfected(j)+1:Nagents];
                % sample = ceil(length(IAgentsAll)*SamplingContactsRand(countSCR:countSCR+X-1,1));
                ...
                %     countSCR = 1;
                % end
                % IndexOfContacts = IAgentsAll(sample)';
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % FAST WAY: Sample X agents with replacement (using 
                ...
                if any(NewInfections)
                    ...
                    % % SLOW:
                    %[IndexOfContacts, temp, ~] = unique(IndexOfContacts);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    if burnin == 0
        if Observation_number <= length(Consultations)
            if i == time_obs(Observation_number)
                Observed_AC_time(:,:,Observation_number) = AgentCharacteristics(:,1:end-1);
                Observed_ImS_time(:,:,Observation_number) = ImmuneStatus;
