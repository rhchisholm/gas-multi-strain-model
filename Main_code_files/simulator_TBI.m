...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Store time since last infection  %%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
TimeLastInfection = -1 * ones(Nagents,Nstrains);
TimeBetweenInfectionSS = [];
...
for i = 1 : Ntimesteps - 1  
         ...
         for j = 1 : length(RowIndexInfected)
            ...
            if X > 0
                ...
                if any(NewInfections)
                    ...
                    if CurrentTime > 98
                        for p = 1 : length(idx)
                            if TimeLastInfection(idx(p))>0
                                TimeBetweenInfectionSS = [TimeBetweenInfectionSS;...
                                    CurrentTime - TimeLastInfection(idx(p))];
                            end
                        end
                    end
    ...
    for h = 1 : NumMig
        ...
        countM = countM + 1;
        TimeLastInfection(D(h),:) = -1 * ones(1,Nstrains);  
    end
